...
import './sign.css';
const Signup = () => {
    const [person, setperson] = useState({ firstname: '', lastname: '',mobile:'',email: '', password: '',t:0 });
    let name,value;
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const {firstname,lastname,mobile,email,password,t}=person
        const res= await fetch('/register',{
            method:'POST',
            headers:{
                'Content-Type':'application/json'
            },
            body:JSON.stringify({firstname,lastname,mobile,email,password,t})
        })
        const data= await res.json()
        if((res.status===400 || !data)||(!firstname || !lastname || !mobile || !email || !password)){
